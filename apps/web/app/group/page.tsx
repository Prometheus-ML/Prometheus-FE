...
import { faPlus, faUsers, faEye, faUserPlus, faCheck, faTimes, faSearch, faUndo, faHeart, faArrowLeft, faImage, faClock, faTrash, faCircle } from '@fortawesome/free-solid-svg-icons';
    ...
    if (groups.length > 0) {
      groups.forEach(async (group) => {
        try {
          ...
          await checkUserMembership(group.id); // 가입 상태 확인 추가
        } catch (error) {
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between pt-2">
                    <button
                      ...
                      className={`flex items-center space-x-1 px-2 py-1 rounded text-sm transition-colors bg-white/10 hover:bg-white/20 ${
                    ...
                    >
                      <FontAwesomeIcon 
                        icon={faHeart} 
                        className={`w-3 h-3 ${
                          userLikedGroups[group.id] ? 'text-red-400' : 'text-gray-400'
                        }`} 
                      />
                        ...
                        onClick={(e) => {
                          ...
                          handleJoinGroup(group.id);
                            ...
                            >
                              ...
                              {group.deadline && new Date(group.deadline) < new Date() ? '마감됨' : '가입'}
                      </button>
                    )}

                          {/* 가입 신청 중 표시 */}
                          {hasPendingRequest(group.id) && (
                            <span className="text-yellow-400 text-xs px-2 py-1 rounded bg-yellow-500/20">
                              신청 중
                            </span>
                          )}
                          ...
                          {isGroupMember(group.id) && (
                            <span className="text-blue-400 text-xs px-2 py-1 rounded bg-blue-500/20">
                              멤버
                            </span>
                          )}
                        </>
