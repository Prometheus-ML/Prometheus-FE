...
export default function GroupPage() {
  ...
  const handleJoinGroup = async (groupId: number) => {
    if (!confirm('이 그룹에 가입 요청을 보내시겠습니까?')) {
      return;
    }
    ...
    try {
      ...
      console.error('그룹 가입 요청 실패:', err);
      setError('그룹 가입 요청에 실패했습니다.');
    }
  };
  ...
  const handleGroupClick = async (groupId: number) => {
    ...
    try {
      await Promise.all([
        ...
        // 권한이 있는 경우에만 가입 요청 목록을 불러옴 (에러는 무시)
  ...
  const isGroupOwner = (group: any) => {
    return user && group && user.id === group.owner_id;
  };
