...
export default function AdminMemberPage() {
  ...
  }, [isAuthenticated, canAccessManager]);
  ...
  useEffect(() => {
    if (!isAuthenticated() || !canAccessManager()) return;
    loadMembers();
  }, [page, size, filters]);

  // 유틸리티 함수들
  const getGrantClass = (grant: string) => {
    ...
    <div className="p-6">
      ...
      <div className="mt-6 bg-white shadow px-4 py-5 sm:rounded-lg sm:p-6">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          {/* 권한 필터 */}
          <div>
            <label htmlFor="grant" className="block text-sm font-medium text-gray-700">권한</label>
            <select
              id="grant"
              value={filters.grant_filter}
              onChange={(e) => setFilters((f: any) => ({ ...f, grant_filter: e.target.value }))}
              className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
            >
          ...
          {/* 기수 필터 */}
          <div>
            <label htmlFor="gen" className="block text-sm font-medium text-gray-700">기수</label>
            <select
              id="gen"
              value={filters.gen_filter}
              onChange={(e) => setFilters((f: any) => ({ ...f, gen_filter: e.target.value }))}
              className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
            >
              <option value="">전체</option>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setFilters((f: any) => ({ ...f, status_filter: e.target.value }))}
              className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
            >
              <option value="">전체</option>
      ...
      <div className="mt-6 bg-white shadow overflow-hidden sm:rounded-md">
        {isLoading ? (
          ...
          <ul className="divide-y divide-gray-200">
            {list.map((member) => (
              <li key={member.id} className="px-4 py-4 sm:px-6 hover:bg-gray-50 cursor-pointer" onClick={() => viewMember(member)}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      ...
                      <p className="text-sm text-gray-500">{member.gen}기 · {member.school} {member.major}</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
                    <button
