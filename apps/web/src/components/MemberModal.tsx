...
export default function MemberModal({ isOpen, member, onClose, onSubmit, onDelete }: MemberModalProps) {
  ...
  const canModifyEntireMember = () => {
    ...
    const currentLevel = grantHierarchy[currentUserGrant as keyof typeof grantHierarchy] || 0;
    const memberLevel = grantHierarchy[form.grant as keyof typeof grantHierarchy] || 0;
  ...
  // 권한 할당 가능한지 확인
  const canAssignGrant = (grant: string) => {
    const currentUserGrant = authStore.user?.grant || 'Member';
    const currentLevel = grantHierarchy[currentUserGrant as keyof typeof grantHierarchy] || 0;
    const targetLevel = grantHierarchy[grant as keyof typeof grantHierarchy] || 0;
  ...
  // 활동 기수 제거 함수
  const removeActiveGen = (gen: number) => {
    setForm(prev => ({
      ...prev,
  ...
  const onProfileImageFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        {/* 배경 오버레이 */}
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" onClick={!isUploadingImage ? closeModal : undefined} />
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className={`mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm ${
                            isEdit && !canModifyEntireMember() ? 'bg-gray-100 cursor-not-allowed' : ''
                          }`}
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <div className="mt-1 flex space-x-1">
                          <input
                            ...
                            maxLength={3}
                            placeholder="010"
                            className={`w-16 border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-center ${
                              isEdit && !canModifyEntireMember() ? 'bg-gray-100 cursor-not-allowed' : ''
                            }`}
                          />
                          ...
                          <input
                            ...
                            maxLength={4}
                            placeholder="1234"
                            className={`w-20 border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-center ${
                              isEdit && !canModifyEntireMember() ? 'bg-gray-100 cursor-not-allowed' : ''
                            }`}
                          />
                          <span className="flex items-center text-gray-500 text-sm">-</span>
                        ...
                        <div className="mt-2 flex items-center space-x-4">
                          ...
                          <div className="flex-1">
                            ...
                            <button
                              ...
                              className="inline-flex items-center px-3 py-2 border border-gray-300 rounded-md shadow-sm text-sm leading-4 font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                              {!isUploadingImage ? (
                ...
                {activeTab === 'club' && (
                  <div className="mt-6">
                    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                      ...
                      {/* 권한 */}
                      <div>
                        <label htmlFor="grant" className="block text-sm font-medium text-gray-700">
                      ...
                      {/* 활동 시작일 */}
                      <div>
                        <label htmlFor="activity_start_date" className="block text-sm font-medium text-gray-700">활동 시작일</label>
                        <input
                      ...
                      {/* 활동 기수 */}
                      <div className="sm:col-span-2">
                        <label className="block text-sm font-medium text-gray-700">활동 기수</label>
                        <div className="mt-2">
                          {/* 입력 필드 */}
                            ...
                            {form.active_gens.map((gen) => (
                              ...
                              >
                                ...
                                <button
                                  ...
                                  disabled={isEdit && !canModifyEntireMember()}
                                  className="ml-2 inline-flex items-center justify-center w-4 h-4 rounded-full text-blue-600 hover:bg-blue-200 focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                      ...
                      <div className="sm:col-span-2">
                        <div className="flex justify-between items-center">
                          <label htmlFor="history" className="block text-sm font-medium text-gray-700">이력</label>
                          <span className="text-sm text-gray-500">{historyInput.length}/300</span>
                        </div>
                        <div className="mt-2">
                          {/* 입력 필드 */}
                          <div className="flex items-center space-x-2 mb-2">
                            ...
                            {form.history.map((item, index) => (
                              ...
                              >
                                ...
                                <button
                                  ...
                                  disabled={isEdit && !canModifyEntireMember()}
                                  className="ml-2 inline-flex items-center justify-center w-4 h-4 rounded-full text-green-600 hover:bg-green-200 focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                  <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
                                  </svg>
                ...
                {activeTab === 'optional' && (
                  <div className="mt-6">
                    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                      ...
                      {/* Notion */}
                      <div>
                        <label htmlFor="notion" className="block text-sm font-medium text-gray-700">Notion</label>
                        <input
                          id="notion"
                          value={form.notion}
                      ...
                      {/* Figma */}
                      <div>
                        <label htmlFor="figma" className="block text-sm font-medium text-gray-700">Figma</label>
                        <input
                          id="figma"
                          value={form.figma}
                      ...
                      {/* 카카오 ID */}
                      <div>
                        <label htmlFor="kakao_id" className="block text-sm font-medium text-gray-700">카카오 ID</label>
                        <input
                          id="kakao_id"
                          value={form.kakao_id}
                      ...
                      {/* 인스타그램 ID */}
                      <div>
                        <label htmlFor="instagram_id" className="block text-sm font-medium text-gray-700">인스타그램 ID</label>
                        <input
                          id="instagram_id"
                      ...
                      <div className="flex space-x-4">
                        ...
                        <div className="flex-1">
                          <label htmlFor="coffee_chat_enabled" className="block text-sm font-medium text-gray-700">커피챗 활성화</label>
                          <select
                      ...
                      <div className="sm:col-span-2">
                        <div className="flex justify-between items-center">
                          <label htmlFor="self_introduction" className="block text-sm font-medium text-gray-700">자기소개</label>
                          <span className="text-sm text-gray-500">{form.self_introduction.length}/300</span>
                        </div>
                        <textarea
                      ...
                      <div className="sm:col-span-2">
                        <div className="flex justify-between items-center">
                          <label htmlFor="additional_career" className="block text-sm font-medium text-gray-700">추가 경력</label>
                          <span className="text-sm text-gray-500">{form.additional_career.length}/300</span>
                        </div>
                        <textarea
